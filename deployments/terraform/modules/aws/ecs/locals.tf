...
locals {
  ...
  ecs_cluster_name = var.ecs_cluster_name != null ? var.ecs_cluster_name : "${local.resource_prefix}-cluster"
  ecs_service_name = var.ecs_service_name != null ? var.ecs_service_name : local.resource_prefix

  iam_task_role_name      = var.iam_task_role_name != null ? var.iam_task_role_name : "${local.resource_prefix}-ecs-task"
  iam_execution_role_name = var.iam_execution_role_name != null ? var.iam_execution_role_name : "${local.resource_prefix}-ecs-exec"
  ...
  cloudwatch_log_group_name        = var.cloudwatch_log_group_name != null ? var.cloudwatch_log_group_name : "/aws/ecs/${local.ecs_cluster_name}/${local.ecs_service_name}"

  # ALB naming
  alb_name          = var.alb_name != null ? var.alb_name : "${local.resource_prefix}-alb"
  target_group_name = "${local.resource_prefix}-tg"
  ...
  elasticache_subnet_ids = length(var.elasticache_subnet_ids) > 0 ? var.elasticache_subnet_ids : var.private_subnet_ids
  ...
  default_tags = merge(
    {
      ...
      "deployment-type" = "ecs"
      "ecs-cluster"     = local.ecs_cluster_name
      "ecs-service"     = local.ecs_service_name
    },
    var.tags
  )
}
